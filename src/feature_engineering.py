...
# ----------------------------------------------------
# This class defines a common interface for different feature engineering techniques
class FeatureEngineeringStrategy(ABC):
    @abstractmethod
    def apply_transformation(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Abstract method to apply feature engineering formation to the DataFrame.
...
# ----------------------------------------
# This strategy applied a Logarithmic transformation to skewed features to normalize the distribution.
...
# --------------------------------------
# This strategy applied standard scaling(z-score normalization) to the features, centering them around zero with unit variance.
...
# -------------------------------------
# This Strategy applied Min-Max scaling to features, scaling them to a specified range, typically [0,1].
...
class OneHotEncoding(FeatureEngineeringStrategy):
    ...
    def apply_transformation(self, df: pd.DataFrame):
        logging.info(f"Applyting One-hot encoding to features: {self.features}")
        df_transformed = df.copy()
...
if __name__ == "__main__":
    ...
    # df_log_tranformed = log_transformer.apply_feature_engineering(df)
    ...
    # df_minmax_transformed = MinMax_scaler.apply_feature_engineering(df)

    # # OneHot Ending
    # OneHot_encoder = FeatureEngineer(OneHotEncoding(features=['Neighborhood']))
    # df_onhot_encoded = OneHot_encoder.apply_feature_engineering(df)
